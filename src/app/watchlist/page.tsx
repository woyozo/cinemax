...
export default function WatchlistPage() {
  const { watchlist, removeFromWatchlist } = useCinemaxStore()

  return (
    <div className="min-h-screen bg-cinemax-dark pt-20">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-3 mb-8">
          <Bookmark className="w-7 h-7 text-cinemax-red" />
        ...
        {watchlist.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-gray-500 mb-6">Save movies and shows to watch later</p>
            <Link
              href="/"
                ...
                <Link href={`/${item.media_type === 'movie' ? 'movies' : 'tv-shows'}/${item.media_id}`}>
                  <div className="aspect-poster rounded-lg overflow-hidden bg-cinemax-dark-3 relative">
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault()
                        removeFromWatchlist(item.media_id, item.media_type)
                      }}
